...
class RequestQuotationController extends ControllerMVC {
  ...
  void  requuestquotation(BuildContext context) {
    setState(() {
      loading=true;
    });
    requestquotation(orderModel.id).then((value) {
      print(value);
      setState(() {
        loading=false;
        if(value!=null){
          Toast.show("Request added successfully", context, duration: Toast.LENGTH_LONG, gravity:  Toast.CENTER);

          this.orderModel=value;
          Navigator.pushReplacement(context,
              CupertinoPageRoute(builder: (context) => TrackingWidget(index: 1,orderModel: value,)));
          scaffoldKey?.currentState?.showSnackBar(SnackBar(
            content: Text("Quotation Requested "),
          ));
        }
        else{
          scaffoldKey?.currentState?.showSnackBar(SnackBar(
            content: Text("Failed To request  quotation"),
          ));
